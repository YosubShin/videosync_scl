...
def get_embeddings_dataset(cfg, model, data_loader, output_dir):
    ...
    with torch.no_grad():
        for video, frame_label, seq_len, _, _, names in data_loader:
            ...
            mask_for_UB_S1 = torch.bitwise_and(
                (frame_label[0] >= 74), (frame_label[0] <= 88))
...
def evaluate_once(cfg, model, train_loader, val_loader, train_emb_loader, val_emb_loader,
    ...
    synchronize()
    logger.info(
        f"generating train embeddings for finegym dataset at {train_output_dir} of epoch {cur_epoch}.")
    train_files, train_oneset_dataset = get_embeddings_dataset(
        cfg, model, train_emb_loader[0], train_output_dir)
    if cfg.NUM_GPUS > 1:
        train_files = list(chain(*all_gather_unaligned(train_files)))
        train_oneset_dataset = list(
            chain(*all_gather_unaligned(train_oneset_dataset)))
    ...
    synchronize()
    logger.info(
        f"generating val embeddings for finegym dataset at {val_output_dir} of epoch {cur_epoch}.")
    val_files, val_oneset_dataset = get_embeddings_dataset(
        cfg, model, val_emb_loader[0], val_output_dir)
    if cfg.NUM_GPUS > 1:
        val_files = list(chain(*all_gather_unaligned(val_files)))
        val_oneset_dataset = list(
            chain(*all_gather_unaligned(val_oneset_dataset)))
    ...
    for fraction in fractions:
        batch_size = 10
        num_train = max(cfg.NUM_GPUS*batch_size,
                        int(fraction * len(train_files)))
        train_embs = FinegymEval(cfg, 'train', train_files[:num_train])
        train_sampler = torch.utils.data.distributed.DistributedSampler(
            train_embs) if cfg.NUM_GPUS > 1 else None
        ...
        val_sampler = torch.utils.data.distributed.DistributedSampler(
            val_embs) if cfg.NUM_GPUS > 1 else None
        ...
        model = LogisticRegression(
            cfg.MODEL.EMBEDDER_MODEL.EMBEDDING_SIZE, cfg.EVAL.CLASS_NUM)
        model = model.cuda()
        ...
        optimizer = torch.optim.SGD(
            model.parameters(), lr=learning_rate, momentum=0.9, weight_decay=1e-6)
        ...
        for e in range(total_e):
            ...
            for param_group in optimizer.param_groups:
                param_group["lr"] = learning_rate * \
                    (1 + math.cos(math.pi * e/(1.0*total_e)))/2
            correct = 0.0
            total = 0.0
            ...
            if e % 10 == 0:
                logger.info(
                    f'[{e}/{total_e}] classification_{fraction} train set: {train_accuracy:.3f}% ({correct}/{total})')
            ...
            if e % 10 == 0:
                logger.info(
                    f'[{e}/{total_e}] classification_{fraction} val set: {accuracy:.3f}% ({correct}/{total})')

        summary_writer.add_scalar(
            f'classification_{fraction}/train', train_accuracy, cur_epoch)
        summary_writer.add_scalar(
            f'classification_{fraction}/val', accuracy, cur_epoch)
        torch.cuda.empty_cache()

    if cfg.EVAL.CLASS_NUM == 99 and is_root_proc():
        evaluate_oneset_data(cfg, val_emb_loader, cur_epoch,
                             summary_writer, val_oneset_dataset)
...
def evaluate_oneset_data(cfg, val_emb_loader, cur_epoch, summary_writer, val_oneset_dataset):
    ...
    for data in val_emb_loader[0].dataset.dataset:
        if data['name'] == query_data['name']:
            query_video = val_emb_loader[0].dataset[data["id"]
                                                    ][0][query_data['mask']].permute(0, 2, 3, 1)
        elif data['name'] == key_data['name']:
            key_video = val_emb_loader[0].dataset[data["id"]
                                                  ][0][key_data['mask']].permute(0, 2, 3, 1)
        ...
        for k in range(K):
            ...
            for data in val_emb_loader[0].dataset.dataset:
                if data['name'] == ret_data['name']:
                    video = val_emb_loader[0].dataset[data["id"]][0][ret_data['mask']].permute(0, 2, 3, 1)[
                        ::time_stride]
                    retrieval_frames[k].append(
                        video[topk[t, k]-number[video_i]])
                    break
