...
def float_feature(v): return feature(float_list=tf.train.FloatList(value=v))
...
def get_example(name, seq, seq_label=None, label_string=None,
    ...
    if frame_labels_string:
        ...
        context_features_dict['framelabels_string'] = bytes_feature([
                                                                    labels_string])
...
def create_tfrecords(name, output_dir, input_dir, label_file, input_pattern,
    ...
    if not isinstance(input_pattern, list):
        ...
        for file_pattern in input_pattern:
            ...
            filenames += [os.path.basename(x)
                          for x in gfile.glob(file_pattern)]
    ...
    for i, filename in enumerate(filenames):
        ...
        if label_file is not None:
            ...
            if video_id in data:
                ...
                raise ValueError(
                    'Video id %s not found in labels file.' % video_id)
            merged_annotations = merge_annotations(video_labels,
                                                   expected_segments)

            seqs['labels'] = label_timestamps(
                video_timestamps, merged_annotations)
