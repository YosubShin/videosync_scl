...
def check_video_corruption(file_path):
    """
    Use ffprobe to check if the video file is corrupted.
    Returns True if the file is corrupted, False otherwise.
    """
    try:
        result = subprocess.run(['ffprobe', file_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def find_corrupted_files(target_dir, target_filenames, max_workers=4):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_filename = {executor.submit(check_video_corruption, os.path.join(target_dir, filename)): filename for filename in target_filenames}
...
def main(csv_path, target_dir, max_workers=4):
    ...
    corrupted_files = find_corrupted_files(target_dir, target_filenames, max_workers)

    print("Corrupted files:")
    print('Total corrupted files:', len(corrupted_files))
    print('Percentage of corrupted files:', len(corrupted_files) / len(target_filenames) * 100)
